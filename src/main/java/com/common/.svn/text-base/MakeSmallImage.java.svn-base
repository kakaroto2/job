...
public class MakeSmallImage {
	...
	public String compressPic(File uploadFile, String outputDir,
		...
		// 是否是等比缩放 标记
		this.proportion = gp;
		return compressPic(uploadFile);

	}
	public String compressPic(File file) {
		File absoluteFolder = new File(outputDir);
	...
	public static void main(String[] arg) {
		MakeSmallImage mypic = new MakeSmallImage();
		System.out.println("输入的图片大小：" + mypic.getPicSize("E:\\tupian\\1.jpg")
				/ 1024 + "KB");
		int count = 0; // 记录全部图片压缩所用时间
		for (int i = 0; i < 100; i++) {
			int start = (int) System.currentTimeMillis(); // 开始时间
			mypic.compressPic("E:\\tupian\\", "E:\\tupian\\test\\", "1.jpg",
					"r1" + i + ".jpg", 120, 120, true);
